...
Sources:
- https://hub.docker.com/_/zig
- https://ziglang.org/download/
- https://github.com/ziglang/zig/releases
<br><br>

It can be installed the most known **ZIG** frameworks:

- [Spider](https://www.spiderme.org/)
- [JetZig](https://www.jetzig.dev/)
- [Ziex](https://ziex.dev/)
- [Zylix](https://www.zylix.dev/)
- [Horizon](https://harmonicom.github.io/horizon/)
- Others... [https://zigistry.dev/](https://zigistry.dev/)
<br>
...
COMPOSE_PROJECT_IMGK="alpine-3.24-nginx-zig"            # <- real main image keys to manage automations for sharing resources -------------------------------> #
...
To automatically run the ZIG application, create the Supervisord service that runs the application. You can choose the dev or production version, and set it according to your project requirements

- `./docker/config/supervisor/conf.d-sample/zig-api-dev.conf` -> `./docker/config/supervisor/conf.d/zig-api-dev.conf`

```bash
[program:zig-api-dev]
command=bash -c "zig build run --watch"
directory=/var/www
autostart=true
autorestart=true
stdout_logfile=/var/log/supervisor/zig_api.log
stderr_logfile=/var/log/supervisor/zig_api.err
```
<br>

To support environment-specific differences, there is a sample supervisor configuration directory:
`./platform/nginx-zig/docker/config/supervisor/conf.d-sample`

Before building the container you must copy that directory to:
`./platform/nginx-zig/docker/config/supervisor/conf.d`

Make sure the copied conf.d contains at least the service files needed to run Nginx and Zig (for example, supervisor program entries for Nginx and Zig).

This approach lets developers run additional worker processes locally without changing the shared platform settings. If you need to update Nginx, Zig, or supervisord configurations on a running container, there are Makefile recipes in `./platform/nginx-zig/Makefile` that can apply changes *(reload or update services)* without destroying and rebuilding the container. Check the Makefile for available targets and usage by executing `$ make help` in its directory.
<br><br>
